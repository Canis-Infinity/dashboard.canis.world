            ...
            <div className="grid max-h-[65svh] overflow-y-auto">
              ...
              <section className="grid gap-4 border-t px-6 py-5">
                ...
                <Tabs defaultValue="zh-TW">
                  ...
                  <TabsContent value="zh-TW" className="grid gap-4 pt-2"><div className="grid gap-2"><Label htmlFor="title-zh">標題</Label><Input id="title-zh" value={draft.title?.['zh-TW'] || ''} onChange={(event) => updateLocalized('title', 'zh-TW', event.target.value)} required /></div><div className="grid gap-2"><Label htmlFor="description-zh">說明</Label><Textarea id="description-zh" className="min-h-28" value={draft.description?.['zh-TW'] || ''} onChange={(event) => updateLocalized('description', 'zh-TW', event.target.value)} /></div></TabsContent>
                  <TabsContent value="en" className="grid gap-4 pt-2"><div className="grid gap-2"><Label htmlFor="title-en">Title</Label><Input id="title-en" value={draft.title?.en || ''} onChange={(event) => updateLocalized('title', 'en', event.target.value)} required /></div><div className="grid gap-2"><Label htmlFor="description-en">Description</Label><Textarea id="description-en" className="min-h-28" value={draft.description?.en || ''} onChange={(event) => updateLocalized('description', 'en', event.target.value)} /></div></TabsContent>
                </Tabs>
              </section>
              <section className="grid gap-4 border-t px-6 py-5">
